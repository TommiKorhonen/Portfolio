...
const Contact = () => {
  ...
  return (
    <section
      ...
      name="contact"
    >
      <Title title={"Contact"} mb={"mb-12"} />
      <div
        className={
          formIsSumbitted
            ? "centerFlex flex-col text-center bg-green-400 rounded-md p-4 min-w-96"
            : "hidden"
        }
      >
      ...
      <div className="px-8 rounded-md centerFlex flex-col w-full max-w-3xl mx-auto">
        <form className="p-8 flex flex-col gap-4 w-full" onSubmit={sendEmail}>
          <label htmlFor="name">
            ...
            />
            {errors.name && (
              <p className="mt-3 text-white bg-red-500 w-40 text-center rounded-sm">
                {errors.name}
              </p>
            )}
          </label>
          ...
          <label htmlFor="message">
            ...
            <textarea
              ...
              onChange={handleChange}
              className="rounded-md bg-transparent border border-solid border-twitchPurple p-2 resize-none w-full max-w-2xl"
            ></textarea>
            {errors.message && (
              <p className="mt-3 text-white bg-red-500 w-40 text-center rounded-sm">
                {errors.message}
